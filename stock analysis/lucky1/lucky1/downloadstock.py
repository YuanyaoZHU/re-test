...
@ProjecInformation: 本项目用于对股票的全面策略回测
"""
...
class Stock:
    def __init__(self, code, start='19901219', end=''):
        self.code = code
        self.start = start
        self.end = end
    ...
    def get_daily(self):
        ...
        url = url_mod % (self.code, self.start, self.end)
...
class readBatch:
    ...
    def rerange_stockID(self):
        ...
        for inx, x in enumerate(stock_ID):
            ...
            q = x[0:2]
            if q[1] == 'H':
                self.stock_ID.iat[inx] = "0" + p
            elif q[1] == 'Z':
                self.stock_ID.iat[inx] = "1" + p
...
class Analysis:
    ...
    def method9(self):
        ...
        self.account.stock.df.loc[(self.account.stock.df['dm60'] * self.account.stock.df['dm60'].shift(1) < 0), '5_60cross'] = True
        self.account.stock.df.loc[(self.account.stock.df['dm20']*self.account.stock.df['dm20'].shift(1) < 0), '5_20cross'] = True
        self.account.stock.df.loc[(self.account.stock.df['dm10']*self.account.stock.df['dm10'].shift(1) < 0), '5_10cross'] = True
        self.account.stock.df.loc[(self.account.stock.df['MA60']-self.account.stock.df['MA60'].shift(1) > 0), '牛市'] = True
        ...
        for i in self.account.stock.df.index:
            ...
            if not self.account.hold and close > MA60 and self.account.stock.df.loc[i, '5_60cross'] and self.account.stock.df.loc[i, '牛市']:
                self.account.buy(i)
            elif self.account.hold and close < MA60:
                self.account.sell(i)
            elif self.account.hold and close < 0.97*self.account.price:
                self.account.sell(i)
            self.account.monitor(i)
        return self.account
